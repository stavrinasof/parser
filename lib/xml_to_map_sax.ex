defmodule XmlToMapSax do
  ...
  defp parse_attributes([{:attribute, attrname, [], [], attrvalue}|attrlist],attrmap) do
    attrmap = Map.put(attrmap,attrname,attrvalue)
  ...
  def parse_element({:startElement, [], tagname, [], attrlist} ,acc_tuples) do
    [{tagname,parse_attributes(attrlist,%{})} |acc_tuples ]
  end

  def parse_element({:characters, characters} ,acc_tuples) ,do: [{:chars,characters}|acc_tuples]
  ...
  def parse_element({:endElement, [], 'ContentAPI'=tagname, []},acc_tuples) do
    ...
    Enum.reject(acc_tuples,fn x -> elem(x,0) ==tagname end)
  end
  def parse_element({:endElement, [], tagname, []},acc_tuples) when tagname in @upperclasses do
    index= Enum.find_index(acc_tuples, fn x -> elem(x,0) ==tagname end)
    map_inner=
      acc_tuples
      |> Enum.at(index)
      |> elem(1)
    rest_tuples= Enum.reject(acc_tuples,fn x -> elem(x,0) ==tagname end)
    [{tagname,%{tagname=> map_inner}}|rest_tuples]
  end
