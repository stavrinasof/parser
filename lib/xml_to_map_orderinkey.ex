defmodule XmlToMapOrderInKey do
  ...
  def parse(content, _) when is_list(content) do
    to_string(content)
  end
  ...
  def parse({name, [], content}, order) when name in @upperclasses do
    do_parse_content(content, [], 0)
  end

  def parse({name, [], content}, order) when name in @lowerclasses do
    key = find_id_from_attributes(name, [])
    %{{key,order} => do_parse_content(content, [], 0)}
  end
