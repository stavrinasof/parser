defmodule MapDiff do
  def diffs(map_a, map_b) when is_map(map_a) and is_map(map_b),
    do: diffs(map_a, map_b, [], %{:added => %{}, :changes => %{},:deleted => map_a})
    ...
    |> Enum.reduce(result, fn k, map_changes ->
      ...
      case Map.get(map_a, k) do
        ...
        map_a_value ->
          ...
          Map.put(new_map_changes,:added, deleted_map_a)
      end
    end)
  end

  defp diffs(map_a_value, map_b_value, new_key_path, map_a_copy_plus)
       when map_a_value == map_b_value do
    map_a_copy = Map.get(map_a_copy_plus,:added)
    {:equals, delete_in(map_a_copy, new_key_path)}
  end

  defp diffs(_map_a_value, _map_b_value, new_key_path,  %{:added => map_a_copy}) do
