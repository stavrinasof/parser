defmodule MapDiff do
  ...
  def diff(a, a), do: %{changed: :equal, value: %{}}
  ...
  def diff(a = %{}, b = %{}) do
    ...
    (equal? && %{changed: :equal, value: %{}}) ||
      %{changed: :map_change, value: changes, removed: a, added: b}
  end
  ...
  #   {Map.put(changes, key, %{changed: :equal, value: %{}}), equal?}
  # end
  defp compare({key, vala}, {changes, equal?}, valb, true) when is_map(vala) and is_map(valb) do
    ...
    case valueDiff.changed do
      # :equal -> {Map.put(changes, key, %{changed: :equal, value: %{}}), equal?}
      :equal ->
        {%{}, equal?}
